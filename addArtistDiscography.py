...
def test_api_request():
  ...
  if token:
    ...
    searchResults = sp.artist_albums('spotify:artist:13y7CgLHjMVRMDqxdx0Xdo', album_type='album',limit=50, offset=0) 
    for items in searchResults['items']:
      album_uri = items['uri']
      album_track = sp.album_tracks(album_uri)
      for item in album_track['items']:
          cat = item['name']
          song = cat + ' - ' + item['artists'][0]['name']
          ...
          d['string{0}'.format(x)]=youtube.playlistItems().insert(
            ...
            body=dict(
              snippet=dict(
                playlistId= '', # enter a valid YouTube playlist ID here
                resourceId=dict(
                  kind= 'youtube#video',
                  videoId= vidId
                )
