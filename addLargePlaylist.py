...
def get_playlist_tracks(username, playlist_id):
      ...
      searchResults = sp.user_playlist_tracks(username, playlist_id, offset=1400) # enter Spotify account username and playlist hash
      tracks = searchResults['items']
      while searchResults['next']:
        searchResults = sp.next(searchResults)
        tracks.extend(searchResults['items'])
        return tracks
...
def test_api_request():
  ...
  if token:
    ...
    gstuff= get_playlist_tracks('', '') # enter Spotify account username and playlist hash
    for xy in gstuff:
      trac = xy['track']
      song= trac['name'] + ' - ' + trac['artists'][0]['name']
      ...
      d['string{0}'.format(x)]=youtube.playlistItems().insert(
        ...
        body=dict(
          snippet=dict(
            playlistId= '', # enter valid playlist ID
            resourceId=dict(
              kind= 'youtube#video',
              videoId= vidId
            )
